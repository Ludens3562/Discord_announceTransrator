---
description: ワークスペース内のすべてのファイルを分析し、包括的なプロジェクトドキュメントを生成する。
---
...
#### 1.6 自動除外
以下は読み込みをスキップ：
- `media/`, `staticfiles/`, `.github/`
- `.env`, `.git/`, バイナリファイル、ログファイル
...
**存在しない場合：**
- 以下の標準構成を使用する
- 上記構成はあくまで例であり、プロジェクトの内容に応じて適宜セクションを追加・削除しても良い
...
- **配置場所**: `docs/wiki/` ディレクトリ（存在しない場合は自動作成）
- **ファイル命名規則**: 
